...
// color switch timing replacing framerate 
let flowers = [];
let flowerCount = 80;
let isGrowing = false;
let growthSpeed = 0.3; // base growth speed
  ...
  for (let i = 0; i < flowerCount; i++) {
    ...
    while (!validPosition && attempts < 100) {
      newFlower = {
        ...
        growthDelay: random(500, 3500), // milliseconds before growing starts
        growthStarted: false,
        individualSpeed: random(0.1, 0.8) // individual growth speed
      };
...
function drawFlower(flower) {
  ...
  for (let i = 0; i < flower.petalCount; i++) {
    ...
    let satVariation;
    let briVariation;
    let timing = millis() * 0.005; 

    if (flower.size < flower.targetSize) {
    hueVariation = sin(timing + flower.x * 0.01 + flower.y * 0.01) * 15;
    satVariation = cos(timing + flower.x * 0.01) * 5;
    briVariation = sin(timing + flower.y * 0.01) * 5;
    } else {
    hueVariation = sin(timing + flower.x * 0.01 + flower.y * 0.01) * 2;
    satVariation = cos(timing + flower.x * 0.01) * 1;
    briVariation = sin(timing + flower.y * 0.01) * 2;
    }
